...
def parse_cli_args():
    ...
    parser.add_argument("-r", "--region", type=str, help="Region", choices=get_regions(), nargs='?')
...
def set_region(from_args = None, default = 'us-east-1'):
    ...
    if from_args: # Read from CLI args
        region = from_args
    elif get_value_from_user_config('region') is not None: # Read from config file
...
def get_aws_client():
    ...
    # Client connection
    session = boto3.Session(profile_name=get_value_from_user_config('aws_profile_name'))
